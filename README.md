...
| `cari wallet f977` · `cari alamat binance` · `cari wallet 0xda...09d9 di LEVERA` | **Mencari alamat** dari potongan yang diingat (hex di mana pun letaknya, bentuk singkat `0xda...09d9`, atau nama). Dengan `di <token>` pencariannya masuk ke daftar trader token itu — satu-satunya jalan ketika yang terlihat cuma beberapa karakter; chain boleh ikut disebut (`di LEVERA robinhood`) untuk mempersempit. Semua yang mirip ditampilkan supaya bisa dipilih sendiri. Dijawab KODE tanpa model — alamat karangan bisa membuat dana terkirim ke tempat yang salah |
| `/help` | Bantuan |
