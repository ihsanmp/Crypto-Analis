...
| `token baru` · `token baru base` · `token baru solana` | **Pemindai token baru** (BSC default, Base, Solana): pool yang baru dibuat, diperiksa kontraknya satu per satu. EVM: honeypot, pajak, LP terkunci, konsentrasi. Solana: mint & freeze authority, transfer hook. Dijawab KODE tanpa model, beberapa detik |
| `cari wallet f977` · `cari alamat binance` | **Mencari alamat** dari potongan yang diingat (hex di mana pun letaknya, atau nama). Semua yang mirip ditampilkan supaya bisa dipilih sendiri. Dijawab KODE tanpa model — alamat karangan bisa membuat dana terkirim ke tempat yang salah |
| `/help` | Bantuan |
...
| [cloud/investors.py](cloud/investors.py) | Kepemilikan on-chain multi-chain: 10 holder teratas + kategori otomatis + konsentrasi riil. Ethereum via Ethplorer; Base/Arbitrum/Optimism/Polygon via Blockscout; Avalanche via Routescan; BSC & Solana via GoPlus Security — semuanya tanpa key |
