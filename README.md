...
| `token baru` · `token baru bsc` | **Pemindai token baru di BSC**: pool yang baru dibuat, diperiksa kontraknya satu per satu (honeypot, pajak, LP terkunci, konsentrasi). Dijawab KODE tanpa model, beberapa detik |
| `/help` | Bantuan |
...
| [cloud/tokenbaru.py](cloud/tokenbaru.py) | **Pemindai token baru** (BSC): pool yang baru dibuat (GeckoTerminal) + pemeriksaan kontraknya (GoPlus) — honeypot, pajak, owner, mintable, LP terkunci/tidak, konsentrasi pemegang. **Tanpa skor karangan**: yang keluar daftar temuan berangka, vonisnya dihitung kode. Dipanggil lewat perintah `token baru` di Telegram, TANPA model |
