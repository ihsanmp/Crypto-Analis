...
| [cloud/panggilan.py](cloud/panggilan.py) | **Rekam jejak panggilan eksternal** (Astronacci): lima panggilan dari materi yang dikirim user dicatat di [cloud/data/panggilan_eksternal.json](cloud/data/panggilan_eksternal.json) sebelum hasilnya diketahui, lalu dinilai oleh harga — target vs invalid, klaim waktu dicatat tanpa vonis, klaim periode dinilai saat jatuh tempo. Hanya mengamati, seperti rapor.py |
...
| [periksa-outlook.yml](.github/workflows/periksa-outlook.yml) | manual saja | Mencetak semua blok outlook baru (waktu astro, pola, Minor Swing, panggilan eksternal) dengan data live — bursa diblokir dari jaringan lokal pengembang |
